...
// 尝试解析任何结构的时间字符串
func (t *Time) ParseAll(sourceTime string) (time.Time, error) {
	if sourceTime == "" {
		return time.Time{}, nil
	}
	theTime, err := time.ParseInLocation("2006-01-02 15:04:05", sourceTime, time.Local)
	if err != nil {
		theTime, err = time.ParseInLocation("2006-01-02", sourceTime, time.Local)
		if err != nil {
...
func (t *Time) UnixMilliAddHour(msec int64, hour time.Duration) int64 {
	return t.UnixMilliTime(msec).Add(hour*time.Hour).UnixNano() / 1e6
}

func (t *Time) AddMinuteToUnixMilli(minute int) int64 {
	return time.Now().Add(time.Duration(minute)*time.Minute).UnixNano() / 1e6
}

func (t *Time) AddMonthToUnixMilli(month int) int64 {
	return time.Now().AddDate(0, month, 0).UnixNano() / 1e6
}
func (t *Time) AddDayToUnixMilli(day int) int64 {
	return time.Now().AddDate(0, 0, day).UnixNano() / 1e6
}
func (t *Time) AddHourToUnixMilli(hour time.Duration) int64 {
	return time.Now().Add(hour*time.Hour).UnixNano() / 1e6
}
...
// ParseHyphenTimeToUnixMsec 字符串转毫秒时间戳
...
func (t *Time) ParseHyphenTimeToUnix(hyphenTime string) (int64, error) {
	return t.ParseTimeToUnix("2006-01-02 15:04:05", hyphenTime)
}
...
func (t *Time) SliceArraySecondTime(startAt time.Time, endAt time.Time, interval time.Duration) ([][2]time.Time, error) {
	tmp, err := t.SliceArrayTime(startAt, endAt, interval)
	if err != nil {
		return nil, err
	}
	var out = make([][2]time.Time, 0)
	for k, v := range tmp {
		if v[0].Format("2006-01-02 15:04:05") == v[1].Format("2006-01-02 15:04:05") {
			continue
		} else {
...
func (t *Time) ParseUnsignedTime(unsignedTime string) (time.Time, error) {
	return t.Parse("20060102150405", unsignedTime)
}

func (t *Time) ShouldFormatUnsignedFromMilli(in int64) string {
	if in == 0 {
		return ""
	}
	timer := t.UnixMilliTime(in)
...
func (t *Time) GetMonthLastDay(ts time.Time) int {
	month := ts.Month()
	year := ts.Year()
	tmpTime := time.Date(year, month+1, 1, 0, 0, -1, 0, t.Location())
	return tmpTime.Day()
}

//func (t *Time) GetDate(ts time.Time, month int, day int) {
//	time.Date(joinYear, joinMonth+time.Month(month), beginDay, 0, 0, 0, 0, t.Location())
//}

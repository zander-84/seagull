...
func TestNewFriendlyProducer(t *testing.T) {
	...
	m := storage.NewMongo(mdb.DB(), "queue_student", 100)
	fp, err := NewFriendlyProducer("172.16.86.160:4150", "queue_student", m, unique.New("LF", "", "aa", checkcode.NewAlpha(3)))
	if err != nil {
...
// go192 test  -timeout 1h -v  -run TestNewFriendlyConsumer
func TestNewFriendlyConsumer(t *testing.T) {
	mdb := mongo.NewMongo(mongo.Conf{
		Host:            "172.16.86.160",
		Port:            "27017",
		MaxPoolSize:     100,
	...
	m := storage.NewMongo(mdb.DB(), "queue_student", 100)
	fc, err := NewFriendlyConsumer("172.16.86.160:4150", "queue_student", "test", m, 3)
	if err != nil {
		t.Fatal("nsq start err:" + err.Error())
	}
...
func TestNewFriendlyManager(t *testing.T) {
	mdb := mongo.NewMongo(mongo.Conf{
		Host:            "172.16.86.160",
		Port:            "27017",
		MaxPoolSize:     100,
	...
	_manager := NewManager(fp, "queue_student", m)
	if err := _manager.SendFromBackup("LFMPU925000011230116161342"); err != nil {
		t.Fatal(err.Error())
	}

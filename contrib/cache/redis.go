...
func NewRedisCache(engine *redis.Client, codec contract.Codec, processor contract.Processor) contract.Cache {
	return &redisCache{engine: engine, codec: codec, processor: processor}
}

func (r *redisCache) Exists(ctx context.Context, keys ...contract.CacheKey) (bool, error) {
...
func (r *redisCache) Get(ctx context.Context, key contract.CacheKey, recPtr any) (err error) {
	defer func() {
...
func (r *redisCache) Set(ctx context.Context, key contract.CacheKey, value any, expires time.Duration) error {
...
func (r *redisCache) SetNX(ctx context.Context, key contract.CacheKey, value any, expires time.Duration) (bool, error) {
	b, err := r.codec.Marshal(value)
	if err != nil {
		return false, err
	}
...
func (r *redisCache) Delete(ctx context.Context, keys ...contract.CacheKey) error {
	var rdsKey = make([]string, 0, len(keys))
	for _, v := range keys {
		rdsKey = append(rdsKey, v.Key())
	}
...
func (r *redisCache) DelayDelete(ctx context.Context, delay time.Duration, keys ...contract.CacheKey) error {
	r.processor.Go(func() {
		select {
...
func (r *redisCache) GetOrSet(ctx context.Context, key contract.CacheKey, recPtr any, expires time.Duration, f func(key contract.CacheKey) (value any, err error)) (err error) {
	defer func() {
		if recoverErr := recover(); recoverErr != nil {
			buf := make([]byte, 64<<10)
			n := runtime.Stack(buf, false)
...
func (r *redisCache) BatchGetOrSet(ctx context.Context, keys []contract.CacheKey, recPtr any, expires time.Duration, f func(missIds []contract.CacheKey) (value map[contract.CacheKey]any, err error)) error {
	...
	for key, val := range missVal {
		if _, err := r.SetNX(ctx, key, val, expires); err != nil {
			return err
		}

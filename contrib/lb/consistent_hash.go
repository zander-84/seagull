...
type consistentHash struct {
	...
	hashMap    map[uint32]any //节点哈希和Key的map,键是hash值，值是节点key
...
func (ch *consistentHash) Update() {
	...
	if atomic.LoadUint64(&ch.version) == ch.listener.Version() {
		return
	}
	...
	for _, node := range addrSlice {
		for i := 0; i < ch.replicas; i++ {
			hash := DefaultHash([]byte(strconv.Itoa(i) + fmt.Sprintf("%v", node)))
...
func (ch *consistentHash) Next() (any, error) {
	return ch.Get(fmt.Sprintf("%d", time.Now().UnixNano()))
}

func (ch *consistentHash) Get(uid any) (any, error) {
	if atomic.LoadUint64(&ch.version) != ch.listener.Version() {
		ch.Update()
	}
	ch.lock.RLock()
	...
	hash := DefaultHash([]byte(fmt.Sprintf("%v", uid)))
...
func (ch *consistentHash) record(data any) {
	ch.recordLock.Lock()
...
func (ch *consistentHash) All() ([]any, error) {
	if atomic.LoadUint64(&ch.version) != ch.listener.Version() {
		ch.Update()
	}
	ch.lock.RLock()
...
func (ch *consistentHash) Used() map[any]int64 {
	ch.recordLock.RLock()

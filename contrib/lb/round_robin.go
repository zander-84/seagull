...
func NewRoundRobin(l Listener, isRecord bool) contract.Balancer {
	rr := &roundRobin{
		listener:     l,
		currentIndex: 0,
		indexMax:     100000000,
...
func (rr *roundRobin) Update() {
	...
	if atomic.LoadUint64(&rr.version) == rr.listener.Version() {
		return
	}
...
func (rr *roundRobin) Next() (any, error) {
	if atomic.LoadUint64(&rr.version) != rr.listener.Version() {
		rr.Update()
	}
	rr.lock.RLock()
...
func (rr *roundRobin) Get(uid any) (any, error) {
	return rr.Next()
}

func (rr *roundRobin) All() ([]any, error) {
	if atomic.LoadUint64(&rr.version) != rr.listener.Version() {
		rr.Update()
	}
	rr.lock.RLock()
...
func (rr *roundRobin) Used() map[any]int64 {
	rr.recordLock.RLock()
	defer rr.recordLock.RUnlock()
	return rr.used
}

func (rr *roundRobin) record(data any) {
	rr.recordLock.Lock()
